...
void testMatrixVectorCSR() {
	...
	MatrixVectorCSR << < GRID_DIM, BLOCK_DIM >> > (M, d_IRP, d_JA, d_AS, d_x, d_y);
...
void testMatrixVectorELLPACK() {
	...
	double *d_AS, *d_x, *d_y;
	int *d_JA;
	checkCudaErrors(cudaMalloc((void**)&d_AS, M * MAXNZ * sizeof(double)));
	checkCudaErrors(cudaMalloc((void**)&d_x, N * sizeof(double)));
	checkCudaErrors(cudaMalloc((void**)&d_y, M * sizeof(double)));
	checkCudaErrors(cudaMalloc((void**)&d_JA, M * MAXNZ * sizeof(int)));

	// Copy matrices from the host (CPU) to the device (GPU).
	checkCudaErrors(cudaMemcpy(d_AS, AS, M * MAXNZ * sizeof(double), cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_x, x, M * sizeof(double), cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(d_JA, JA, M * MAXNZ * sizeof(int), cudaMemcpyHostToDevice));

	// Calculate the dimension of the grid of blocks (1D) necessary to cover all rows. 
	const dim3 BLOCK_DIM(256);
	const dim3 GRID_DIM((M - 1 + BLOCK_DIM.x) / BLOCK_DIM.x, 1);

	MatrixVectorELLPACK << < GRID_DIM, BLOCK_DIM >> > (M, N, MAXNZ, d_JA, d_AS, d_x, d_y);
